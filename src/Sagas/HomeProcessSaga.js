...
import { getAuthTokenSuccess, getAuthTokenFailure} from '../reducers/AuthenticationSlice';
/***************** - GET AUTH CODE SAGA - *************************/
export function* fetchUserAuthTokenSaga(action) {
    try {
        const authCode = yield call(fetchUserAuthentication, action.payload);
        yield put(getAuthTokenSuccess(authCode));
    } catch (error) {
        yield put(getAuthTokenFailure(error.toString()));
    }
